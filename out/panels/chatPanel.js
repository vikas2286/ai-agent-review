...
class ChatPanel {
    static render(context) {
        if (ChatPanel.currentPanel) {
            ...
            const panel = vscode.window.createWebviewPanel('aiChat', '💬 AI Code Chat', vscode.ViewColumn.Beside, {
                enableScripts: true,
    ...
    static async handleUserMessage(context, panel, userMessage) {
        // Add user message
        ...
        try {
            ...
            let systemInstruction = prompts_1.SYSTEM_INSTRUCTIONS.CODE_REVIEWER;
            if (editor && ChatPanel.messages.length === 1) {
                // First message - include code context
                const code = editor.document.getText();
                const language = editor.document.languageId;
                systemInstruction = prompts_1.PROMPTS.CHAT_CONTEXT(code, language);
            }
            const response = await aiService.chat(ChatPanel.messages, systemInstruction);
